...
export default function ContactForm() {
  ...
  return (
    <form
      onSubmit={submit}
      className="rounded-lg border border-black/10 bg-white p-5 shadow-sm md:p-7"
    >
      <div className="mb-6 flex items-center gap-3">
        <MessageSquare className="text-champagne" size={22} />
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <textarea
          ...
          className="min-h-36 rounded-md border border-black/15 px-4 py-3 text-sm outline-none md:col-span-2"
        />
        <button
          disabled={loading}
          className="btn-gold gap-2 md:col-span-2 disabled:opacity-60"
        >
          {loading ? (
      ...
      <p className="mt-4 text-xs leading-5 text-black/45">
        Prefer a guided request? Use the custom work form for dimensions,
        materials, budget, and reference uploads.
      </p>
      <Link
        href="/custom-work"
        className="mt-3 inline-flex text-xs font-semibold uppercase tracking-[0.14em] text-champagne"
      >
        Open custom work form
      </Link>
    </form>
  )
}
