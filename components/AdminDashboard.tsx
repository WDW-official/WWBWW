...
const tabs: {
  key: 'overview' | 'orders' | 'products' | 'custom'
  icon: LucideIcon
  label: string
}[] = [
...
export default function AdminDashboard() {
  ...
  const [active, setActive] = useState<
    'overview' | 'orders' | 'products' | 'custom'
  >('overview')
  ...
  async function loadAdminData() {
    setLoading(true)
    setMessage('')
    const [summaryResponse, ordersResponse, requestsResponse] =
      await Promise.all([
    ...
    if (
      [summaryResponse, ordersResponse, requestsResponse].some(
  ...
  if (!adminKey) {
    return (
      <main className="container-luxe grid min-h-[70vh] place-items-center py-12">
        <form
          onSubmit={login}
          className="w-full max-w-md rounded-lg border border-black/10 bg-white p-6 shadow-luxe"
        >
          ...
          <p className="mt-4 text-sm leading-6 text-black/60">
            Enter the admin key from `.env.local` to manage products, orders,
            and custom enquiries.
          </p>
          <input
            required
            type="password"
  ...
  const cards: { label: string; value: string | number; icon: LucideIcon }[] = [
    {
      label: 'Revenue',
      value: summary
        ? formatMoney(summary.stats.revenue, summary.stats.currency)
        : '...',
      icon: CreditCard,
    },
    {
    ...
    { label: 'Products', value: summary?.stats.products ?? '...', icon: Box },
    {
      ...
      value: summary?.stats.customRequests ?? '...',
      icon: Sparkles,
    },
  ]
    ...
    <main className="container-luxe py-8 md:py-12">
      <div className="mb-7 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex gap-2 overflow-x-auto">
          ...
          <button
            onClick={logout}
            className="inline-flex items-center gap-2 rounded-md border border-black/15 bg-white px-4 py-2 text-xs uppercase tracking-[0.12em]"
          >
            <LogOut size={15} />
            Logout
          </button>
      ...
      {loading && (
        <div className="mb-4 inline-flex items-center gap-2 text-sm text-black/55">
      ...
      {(active === 'overview' || active === 'orders') && (
        <section className="grid gap-4">
          {active === 'overview' && (
            <div className="grid grid-cols-2 gap-4 lg:grid-cols-5">
              {cards.map(({ label, value, icon: Icon }) => (
                <div
                  key={label}
                  className="rounded-lg border border-black/10 bg-white p-4 shadow-sm md:p-5"
                >
                  <Icon size={20} className="text-champagne" />
                  <div className="mt-4 text-[10px] uppercase tracking-[0.14em] text-black/45 md:text-xs">
                    {label}
                  </div>
          ...
          <OrderTable
            orders={active === 'overview' ? orders.slice(0, 6) : orders}
      ...
      {active === 'products' && (
        <AdminProductsPanel adminKey={adminKey} onChanged={loadAdminData} />
      )}
          ...
          {requests.map((request) => (
            ...
            >
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                ...
                <span className="w-fit rounded-full bg-[#f1ede6] px-3 py-1 text-xs uppercase tracking-[0.12em]">
                  {request.status || 'new'}
                </span>
              </div>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {request.uploads.map((upload) => (
                    <a
                      ...
                      href={upload.url}
                      target="_blank"
                      className="rounded-md border border-black/15 px-3 py-2 text-xs uppercase tracking-[0.12em]"
                    >
                      View file
                    </a>
                  ))}
...
function OrderTable({
  ...
  orders: Order[]
  onStatusChange: (reference: string, status: string) => void
}) {
    ...
    },
    {
      ...
      render: (order) => (
        <select
          value={order.status}
          onChange={(event) =>
            onStatusChange(order.reference, event.target.value)
          }
          className="rounded-md border border-black/15 bg-white px-3 py-2 text-sm outline-none"
        >
          {statuses.map((status) => (
