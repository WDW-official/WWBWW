...
export default function PaymentCallback() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/paystack/verify?reference=${encodeURIComponent(reference)}`)
      ...
      .catch(() =>
        setStatus(
          'Payment verification failed. Please track your order with the reference.'
        )
      )
  }, [reference])
    ...
    <main className="container-luxe grid min-h-[55vh] place-items-center py-16 text-center">
      <div className="max-w-xl">
        ...
        <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row">
          <Link href="/order-tracking" className="btn-gold">
            Track order
          </Link>
          <Link
            href="/shop"
            className="rounded-md border border-black/20 px-6 py-3 text-xs font-semibold uppercase tracking-[0.16em]"
          >
            Back to shop
          </Link>
        </div>
