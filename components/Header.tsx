  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-50 border-b border-black/10 bg-white/95 backdrop-blur">
        <div className="container-luxe flex h-16 items-center justify-between gap-4 md:h-20">
            ...
            {navItems.map(([label, href]) => {
              ...
              return (
                <Link
                  ...
                  className={`border-b py-1 transition ${active ? 'border-champagne text-champagne' : 'border-transparent hover:border-black/20 hover:text-champagne'}`}
                >
                  {label}
                </Link>
              )
          ...
          <div className="flex items-center gap-4">
            ...
            <Link
              href="/shop"
              className="hidden min-h-11 items-center justify-center rounded-md bg-champagne px-6 py-3 text-xs font-semibold uppercase tracking-[0.16em] text-white shadow-[0_12px_30px_rgba(183,138,69,0.22)] transition hover:bg-[#9e7438] lg:inline-flex"
            >
              Shop now
            </Link>
            <button
      ...
      >
        ...
        />
        <nav
          className={`absolute right-0 top-0 h-full w-[82vw] max-w-sm bg-[#fbfaf7] p-6 shadow-luxe transition duration-300 ${open ? 'translate-x-0' : 'translate-x-full'}`}
        >
            ...
            {navItems.map(([label, href]) => {
              ...
              return (
                <Link
                  ...
                  href={href}
                  aria-current={active ? 'page' : undefined}
                  className={`border-b py-4 text-sm font-medium uppercase tracking-[0.12em] ${active ? 'border-champagne text-champagne' : 'border-black/10'}`}
                >
                  {label}
                </Link>
              )
