  ...
  async function uploadImage(file: File) {
    ...
    try {
      ...
      toast({
        ...
        message: 'The product image is ready.',
      })
    } finally {
      setUploading(false)
    }
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(
        editing ? `/api/admin/products/${product?.id}` : '/api/products',
        {
      ...
      if (!response.ok) {
        ...
        toast({
          type: 'error',
          title: editing ? 'Product not updated' : 'Product not saved',
          message: data.error || 'Please check the product details.',
        })
        return
      }
    } catch (error) {
    ...
    toast({
      type: 'success',
      title: editing ? 'Product updated' : 'Product uploaded',
      message: `${data.product.name} is now in the catalogue.`,
    })
  ...
  return (
    <form onSubmit={submit} className="grid gap-5 md:grid-cols-2">
      ...
      <textarea
        ...
        className="min-h-32 rounded-md border border-black/15 px-4 py-3 text-sm outline-none md:col-span-2"
      />
      ...
      {message && (
        <p className="text-sm text-black/65 md:col-span-2">{message}</p>
      )}
      <div className="flex flex-col gap-3 md:col-span-2 md:flex-row">
        {onCancel && (
          <button
            type="button"
            onClick={onCancel}
            className="inline-flex min-h-11 items-center justify-center rounded-md border border-black/15 px-6 py-3 text-xs font-semibold uppercase tracking-[0.16em]"
          >
            Cancel
          </button>
        )}
        <button
          ...
          className="btn-gold flex-1 gap-2 disabled:opacity-60"
        >
          {saving ? (
